...
for linha in arqEntrada:
    ...
    espacos = 20  # variavel para acrescentar até 20 espaços após o nome
    espacos = espacos - len(colunas[0])
    for i in range(espacos):
        arqSaida.write(" ")
    megaByte = int(colunas[1])/(1024*1024)
    espacos = 10  # Redefinindo os espaços para alinhar a direita
    espacos = espacos - len(str(round(megaByte,2))+" MB")
    for i in range(espacos):
        arqSaida.write(" ")
    arqSaida.write(" %.2f MB " % megaByte)
    porcentagem = float(megaByte/espacoTotal*100)
    espacos = 13  # Redefinindo os espaços para alinhar a direita
    espacos = espacos - len(str(round(porcentagem,2))+" MB")
    for i in range(espacos):
        arqSaida.write(" ")
